  ...
  const renderMenuItems = (items, level = "main") => {
    return items.filter(shouldDisplayItem).map((item) => {
      ...
      return (
        <li
          key={id}
          className={`nxl-item ${hasChildren ? "nxl-hasmenu" : ""} ${
            isActive ? "active" : ""
          } ${openMenus[level] === name ? "nxl-trigger" : ""}`}
        >
          <Link
          ...
          {hasChildren && (
            ...
            >
              {dropdownMenu && renderMenuItems(dropdownMenu, "sub")}
              {subdropdownMenu && renderMenuItems(subdropdownMenu, "nested")}
              {superdropdownMenu && renderMenuItems(superdropdownMenu, "super")}
            </ul>
          )}
        </li>
      );
